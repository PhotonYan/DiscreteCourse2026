...
\textbf{构造思路}：\\
直观上，我们想要构造一个句子 \(\psi\) 说“我具有性质 \(\varphi\)”。我们可以利用**对角线化**的方法。
定义替换函数 \(s(x, y)\)，表示“将公式 \(x\) 中的自由变元替换为 \(y\) 的哥德尔数”。令 \(m = \ulcorner \varphi(s(x, x)) \urcorner\)，然后构造 \(\psi = \varphi(s(\overline{m}, \overline{m}))\)。可以证明，这个 \(\psi\) 恰好满足 \(\vdash \psi \leftrightarrow \varphi(\ulcorner \psi \urcorner)\)。
