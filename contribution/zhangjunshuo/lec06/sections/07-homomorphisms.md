...
参考：[DF] Dummit–Foote《Abstract Algebra》§1.6（定义）、§3.1（Proposition 1 与 Corollary 17）；[DS]《离散数学与结构》§9.5。
...
\begin{tikzpicture}[scale=0.95]
  ...
  \node[below left] at (-2.0,-1.0) {$\ker\pi$};
  \draw[thick,dashed] plot[smooth cycle, tension=0.9] coordinates {(-1.4,0.5) (-1.7,1.3) (-0.9,1.8) (-0.2,1.2) (-0.5,0.6)};
  \node[above] at (-1.0,1.8) {$g\ker\pi$};
  \draw (-2.0,-0.5) .. controls (0.4,-0.2) and (1.8,-0.5) .. (3.5,-0.5);
  \fill (3.55,-0.5) circle (1.6pt);
  \node[right] at (3.6,-0.5) {$e_H$};
  \draw (-0.9,1.3) .. controls (0.8,1.2) and (2.0,0.6) .. (3.5,0.5);
  \fill (3.55,0.5) circle (1.6pt);
  \node[right] at (3.6,0.5) {$\pi(g)$};
  \draw[thick] plot[smooth cycle, tension=0.9] coordinates {(3.1,-0.9) (3.0,0.1) (3.7,0.9) (4.7,0.8) (4.7,-0.2) (4.3,-0.9)};
  \node[below right] at (4.4,-0.8) {$\operatorname{Im}\pi$};
\end{tikzpicture}
\\[4pt]
{\small 图：同态把每个陪集（纤维）压成一点：$\ker\pi$ 压到 $e_H$，陪集 $g\ker\pi$ 压到 $\pi(g)$。}
\end{center}
```
