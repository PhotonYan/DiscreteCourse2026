...
\end{tikzpicture}
\\[4pt]
{\small 图：正方形（$D_8$）的八个对称。上行为旋转 $e,r,r^2,r^3$，下行为关于竖直轴的反射 $s,sr,sr^2,sr^3$；$r$ 是逆时针转 $90^\circ$，$s$ 是左右翻转。}
\end{minipage}
\end{center}
```
...
**相等关系。** 把能通过下面操作互相转化的词视为相等：在词中任意位置插入或删除相邻互逆对，

$$w\,ss^{-1}\,w'\sim w\,w',\qquad w\,s^{-1}s\,w'\sim w\,w'.$$

自由群（free group）$F(S)$ 的元素就是这些词模掉上述等价关系得到的等价类。

**群运算。** 两个类 $[u]$、$[v]$ 的乘积定义为：各取一个代表词 $u,v$，拼接起来，取拼接词所在的类，
...
**自由性。** 自由群中除了由群的性质可推出相等的以外都不相等：例如 $aba^{-1}b^{-1}\ne e$，而 $abb^{-1}a^{-1}=e$。
