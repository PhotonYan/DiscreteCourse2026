...
\begin{itemize}
\item 乘法封闭：$a,b\in H\Rightarrow ab\in H$；
\item 逆元封闭：$a\in H\Rightarrow a^{-1}\in H$。
\end{itemize}
...
\begin{itemize}
\item 结合律：由 $G$ 继承；
\item 单位元：取 $a=b\in H$ 得 $e=aa^{-1}\in H$；
\item 逆元：取 $a=e$ 得 $b^{-1}\in H$；
\item 乘法封闭：于是 $a(b^{-1})^{-1}=ab\in H$。
\end{itemize}
...
\textbf{两个左陪集要么相等，要么不交。} 先看一个元素是否属于 $gH$ 的几种等价说法：
...
\begin{itemize}
\item 无限群不能直接做这种除法：陪集个数要按基数理解。
\item 逆命题方向：$d\mid|G|$ 不保证存在 $d$ 阶子群——正四面体的旋转对称群有 $12$ 个元素，却没有 $6$ 阶子群。
\end{itemize}
