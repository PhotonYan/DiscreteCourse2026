...
**第一步：$h$ 是公因子。** 取 $h=au_0+bv_0$，对 $a$ 作带余除法：
...
**第二步：所有公因子都整除 $h$。** 若 $d\mid a,b$，则 $d\mid au_0+bv_0=h$。特别地，所有正公因子都不超过 $h$。结合第一步，得到

$$
\gcd(a,b)=h=au_0+bv_0.
$$

这就是 **Bézout 恒等式**。它还说明 gcd 是整除偏序中的最大公因子：任意公因子都整除 gcd，而不只是数值上不大于它。$(a,b)=(0,0)$ 时，等式 $0=0u+0v$ 仍成立，但不存在最小正线性组合。

## 两个常用推论

**互素判据。** $\gcd(a,b)=1$ 当且仅当存在整数 $u,v$ 使 $au+bv=1$。

**互素的乘积性质。** 若 $\gcd(a,b)=\gcd(a,c)=1$，取 $au+bv=1$、$as+ct=1$，相乘得
